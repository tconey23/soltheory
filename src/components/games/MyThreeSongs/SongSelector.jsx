...
const SongSelector = ({ token }) => {
  ...
  const handleSave = async (songs) => {
    ...
    let saveSongs = await saveThreeSongs(songs, userMeta?.primary_id)
  ...
  return (
    <Stack direction="column" width="100%" height="100vh" maxHeight="100vh" overflow="hidden" alignItems={'center'}>
      ...
      <Stack width={'95%'} height={'50%'} alignItems={'center'}>
      ...
      {selected?.length > 0 && 
        <Stack height={"50%"} width={'95%'}>
          ...
          <Stack width={'100%'} alignItems={'center'} marginTop={1}>
            {selected?.length === 3 && <Button onClick={() => handleSave(selected, userMeta?.primary_id)}>Save!</Button>}
          </Stack>
          </Stack>
        </Stack>
      }
