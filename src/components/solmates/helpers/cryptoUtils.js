export const generateCipherKey = async () => {
  ...
  export const decryptWithKey = async (base64Data, base64Iv, key) => {
    console.log(base64Data, base64Iv, key)
  ...
  const encrypt = async () => {
    ...
    let cypherKey = await generateCipherKey()
    let impKey = await importKeyFromBase64(cypherKey)

    let encryptedMsg = await encryptWithKey('HI', impKey)

    console.log(cypherKey, impKey, encryptedMsg) 
